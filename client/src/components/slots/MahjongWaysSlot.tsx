...
export const MahjongWaysSlot: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (socket && user?.id) {
      socket.emit('slots:free-spins', { slotId: 'mahjong-ways', userId: user.id }, (res: any) => {
  ...
  // Main spin handler (Mahjong Ways gốc không có Mua Free Spins)
  const handleSpin = () => {
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto flex flex-col items-center select-none text-white pb-12">
      ...
      {/* Paytable & Rules Modal (chuẩn PG Soft Mahjong Ways) */}
      {showPaytableModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-gray-900 border border-emerald-600/50 rounded-2xl p-5 shadow-2xl">
            ...
            {/* Symbols Table */}
            <h4 className="font-bold text-xs uppercase text-gray-400 mb-2">Giá Trị (3 - 4 - 5 Cuộn, 1,024 Ways)</h4>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              {[
                { name: 'Phát (Green)', icon: '發', pay: '15 / 40 / 100' },
                { name: 'Trung (Red)', icon: '中', pay: '12 / 30 / 80' },
                { name: 'Bạch (White)', icon: '白', pay: '10 / 25 / 60' },
                { name: 'Bát Vạn', icon: '捌萬', pay: '8 / 15 / 40' },
                { name: 'Ngũ Đồng', icon: '筒', pay: '5 / 10 / 25' },
                { name: 'Ngũ Sách', icon: '索', pay: '5 / 10 / 25' },
                { name: 'Tam Đồng', icon: '筒', pay: '4 / 8 / 20' },
                { name: 'Nhị Sách', icon: '索', pay: '4 / 8 / 20' }
              ].map(s => (
